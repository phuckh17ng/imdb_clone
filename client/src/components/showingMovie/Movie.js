...
const Movie = ({
	...
	console.log(movieId);
	return (
		<div className="py-3 px-3 h-full flex hover:-translate-x-[6px] hover:-translate-y-[6px] transition-all duration-300 linear hover:shadow-xl">
			...
			<div className="w-full ml-3 flex flex-col justify-between">
				<div className="text-zinc-800">
					...
					<div className="mt-3">
						<div className="text-xl">Director: {director}</div>
						{/* <div className="text-xl mt-2">Actor: {actor}</div> */}
						<div className="text-xl mt-2">Gerne: {gerne}</div>
				...
				<div className="flex justify-between bg-zinc-900/70 rounded-full">
					<Link
						to={trailer}
					...
					<Link
						...
						className="w-[55%] font-bold text-xl text-center h-full bg-amber-500 rounded-full py-3 px-3"
					>
						BUY TICKET
					</Link>
				</div>
