...
const SearchPage = () => {
	...
	return loading ? (
		...
		<div className="w-full bg-black h-full py-6 px-3">
			<div className="w-full max-w-[1250px] px-9 m-auto text-white bg-zinc-800/50 rounded-3xl py-6 flex justify-between">
				<div className="w-3/5 min-w-[720px] pr-3">
								...
								>
									<Link className="flex items-center justify-center">
										<img
											...
											alt={movie.id}
											className="h-[106px] w-[70px] object-center hover:brightness-150"
											to={`/details/${movie.id}`}
										/>
									</Link>
									...
									<div className="px-3 h-full pt-3">
										...
										<div className="text-sm font-thin">{movie.description}</div>
									</div>
								</Link>
							);
						}
							...
							if (movie.id.search("nm") !== -1) {
								return (
									<Link
										...
										to={`/details/${movie.id}`}
									>
										<Link className="w-[112px] rounded-full flex items-center justify-center p-1 border-sky-700/90 bg-sky-700/90">
											<img
												src={
													movie?.image
														? movie.image
												...
												alt={movie.id}
												className="hover:brightness-110 object-cover w-[104px] h-full rounded-full"
												to={`/details/${movie.id}`}
											/>
										</Link>
				...
				<div className="w-1/3">
					...
					<div className="text-zinc-100/70">
						Create a more specific search using a variety of options and filters
					</div>
					<div className="mt-3">
						<ul className="">
